...
void CommunicationTask(void* argument) {
  for (;;) 
    ...
    if (format[0] == 0XAA && format[SIZE - 1] == 0XAA)
    {
		...
		// Gimbal_to_Chassis_Data.chassis_top = (bool)(format[5] & 0x10);//遥控器小陀螺
		Gimbal_to_Chassis_Data.up_ui  = (bool)(format[5] & 0x04);//刷新ui						
		Gimbal_to_Chassis_Data.MCL_of = (bool)format[5];//摩擦轮
		Gimbal_to_Chassis_Data.CM_of  = (bool)(format[5] & 0x01);//仓门
    }
